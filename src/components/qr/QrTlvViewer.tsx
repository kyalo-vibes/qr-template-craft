...
const QrTlvViewer: React.FC<QrTlvViewerProps> = ({ qrString, className = "" }) => {
  ...
  const renderTlvItems = (items: TlvDisplayItem[], level: number = 0) => {
    return (
      <div className={`${level > 0 ? 'pl-6 border-l-2 border-gray-200 ml-2' : ''}`}>
          ...
          <div key={`${item.tag}-${index}-${level}`} className="mb-2">
            <div className="flex items-center">
              <div className="flex items-center bg-white rounded-md shadow-sm border p-1 overflow-hidden">
                <span className="w-12 px-2 py-1 bg-brand-primary text-white text-xs font-mono rounded-l-sm">
                  {item.tag}
                </span>
                <span className="w-12 px-2 py-1 bg-brand-secondary text-white text-xs font-mono">
                  {item.length}
                </span>
  ...
  return (
    <Card className={className}>
      <CardContent className="p-4">
        <h3 className="text-sm font-medium mb-3 text-gray-700">QR TLV Structure</h3>
